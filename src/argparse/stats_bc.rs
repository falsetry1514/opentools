...
#[command(about = "Barcode Statistic", long_about = None)]
...
pub struct StatsBCArgs {
    ...
    /// Output txt file
    #[arg(short = 'o', long, required = true, value_parser = validate_absolute_filepath)]
    output: PathBuf,
...
impl InitStatsBCArgs {
    ...
    pub fn run_stats_cb(&self) -> Result<(), AppError> {
        ...
        let barcode_set = BarcodesIter::new(
            open(self.input_path())?,
            self.pos(),
            self.pattern()
        ).extract_sample_barcodes(0)?;
